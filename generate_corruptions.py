...
def parse_args():
    p = argparse.ArgumentParser(description="Generate endoscopy corruptions for SCARED or Hamlyn datasets")
                    ...
                    for corr in corruption_types:
                        for sev in severities:
                            ...
                            out_dir = os.path.join(
                                output_root, corr, f"severity_{sev}",
                                dataset, keyframe, "data"
                            )
...
#
# Estrategia:
# - recorremos todo input_root
# - tomamos png/jpg/jpeg
# - saltamos cualquier carpeta que tenga "depth"
# - mantenemos la ruta relativa en output_root
# ======================
        ...
        try:
            ...
            for corr in corruption_types:
                for sev in severities:
                    ...
                    out_dir = os.path.join(output_root, corr, f"severity_{sev}", rel_dir)
                    os.makedirs(out_dir, exist_ok=True)
                    out_path = os.path.join(out_dir, img_name)
                    Image.fromarray(img_corr).save(out_path)
...
def main():
    ...
    if args.dataset_type == "scared":
        default_exts = (".jpg",)   # como estaba antes
    else:
        default_exts = (".png", ".jpg", ".jpeg")  # hamlyn es png
